...
public class RecordInformation extends JFrame{
	...
	public void setButtonSure() {
		...
		buttonSure.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(textFieldSearch.getText().length() == 0 && !recAction.Testdate(textFieldDate.getText())) {
	...
	public void setTable() {
		String[] columnNames = {"记录编号","学号","姓名","书号","操作","操作时间","到期时间"};
		Object[][] results = null;
		if(textFieldSearch.getText().length() != 0) {
			try {
				results = recAction.CreateRecTable1(columnNames, textFieldSearch);
			} catch (Exception e) {
				// TODO 自动生成的 catch 块
				e.printStackTrace();
			}
			...
			try {
				results = recAction.CreateRecTable2(columnNames, textFieldDate);
			} catch (Exception e) {
				// TODO 自动生成的 catch 块
				e.printStackTrace();
			}
